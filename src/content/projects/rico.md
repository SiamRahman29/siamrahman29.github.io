...
It's a Python assistant wiring speech-to-text and text-to-speech around a command router. Recognized speech is parsed into an intent, dispatched to the matching handler, and the result is spoken back. Each capability is its own integration — OS-level app launching, third-party web APIs for news/weather/movies, browser automation for YouTube and search, and messaging/email APIs for the outbound actions.

The project also grew a companion CLI (RICA-CLI) exploring the same command surface without the voice layer.
